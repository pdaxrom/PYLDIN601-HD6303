...
int     nxtlab,         /* next avail label # */
        ...
        eof_,            /* set non-zero on final input eof */
...
parse()
        {
        while (eof_==0)          /* do until no more input */
                {
...
{       comment();
        ...
        if(mainflg){            /* do stuff needed for first */
                ol("INCLUDE SYSTEM.H"); /* assembler file */

                zcall("main");  /* call the code generated by small-c */
                pseudoins(43); ol("INT $38");
                }
}
...
;
ch      DB     86      ;switch to assembly
        CLRA
        LDX     #line
        LDAB   lptr+1
        STAB   chXXX+1
chXXX   LDAB   0,X     ;self modifying code !!!!
        ANDB   #$7F
        JMP     RTSC
;
; nch()
...
nch     DB     86
        ...
        INCB
        STAB   nchYYY+1
nchXXX  LDAB   0,X     ;self modifying code !!!!
        ANDB   #$7F
        BNE     nchYYY
        JMP     RTSC
nchYYY  LDAB   0,X     ;self modifying code !!!!
        ANDB   #$7F
        JMP     RTSC
;
; gch()
...
gch     DB     86
        ...
        LDAB   lptr+1
        STAB   gchXXX+1
gchXXX  LDAB   0,X     ;self modifying code !!!!
        ANDB   #$7F
        BEQ     gchYYY
...
; }
;
streq   DB     86      ;switch to assembly
        TSX             ;point to data
...
;
stLooP  LDAA   0,X     ;get char from str2
        BEQ     stRETR  ;if NULL then done
        INX
        STX     stX2
        LDX     stX1    ;pointer to str1
        CMPA   0,X     ;compare chars
        ...
        BRA     stLooP
;
stRETR  PULA           ;return length of string
        JMP     RTSC
;
...
astrLP  DEX
        ...
        LDX     stX1
        LDAA    0,X     ;get char from str1
        BEQ     astrBR  ;NULL ?
        LDX     stX2
        CMPA    0,X     ;compare to str2
        ...
        BRA     astrLP
;
astrBR  PSHB           ;save length
        LDX     stX1
        LDAB   0,X     ;get str1 char
        JSR     alPHA_
        PSHB           ;save result
        LDAB   0,X
        JSR     nuMERIC_
        TSX
        ORAB   0,X     ;set an() flag
        INS             ;clear stack
        BNE     astrR0  ;if set, return NULL
        LDX     stX2
        LDAB   0,X     ;get str2 char
        JSR     alPHA_
        PSHB
        LDAB   0,X
        JSR     nuMERIC_
