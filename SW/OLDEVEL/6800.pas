...
(* Виж п░о╢ед│░а Def_Space. *)
{ В║п░о▒║▓ е дали expression ▓░┐бва да вдига HasLabel ако види '*' }

{ Coditional Compilation added  7.Feb.1991 by YGP }
{ ccSYM = <conditional comiplation symol>  кво да е о▓ <sp> до <sp> или ; }
{ .define ccSYM }
{ .undef  ccSYM }
{ .ifdef  ccSYM }
{ .else         }
{ .endif        }
{ .ifNdef ccsym = об░а▓но▓о на .ifdef ccSYM }
{ виж TurboPascal за под░обно▒▓и по използ│ване▓о }
{ .if <expression> ме дом║░з┐ да нап░ав┐ }

{ Underscore allowed in numbwers 14.Feb.1991 }
{ to allow ldaa #%1111_0000 ; не е ли го▓ а? }
...
(**) const MaxDefined = 16;              { б░ой ▒имволи за │▒ловна компила╢и┐ }
(**) var   CondLevel,
           TrueLevel: word;
           SkipSource: boolean;
...
(**)inc_clevel: array [0..MaxIncl] of word;  { ниво на вложено▒▓ на cond comp }
(**)obj_fname: string[31];
